...
window.view = {
  ...
  activateEvents: function () {
    ...
    this.addClickEvent("optionFor", function () {
      view.clearExecutionSection();
    });
    this.addClickEvent("optionWhile", function () {
      view.clearExecutionSection();
    });
    this.addClickEvent("optionDoWhile", function () {
      view.clearExecutionSection();
    });
    this.addClickEvent("btnStart", function () {
      view.startBtn();
    });
    this.addClickEvent("nestedStartBtn", function () {
      view.startBtnNested();
    });
    this.addClickEvent("nestedNextBtn", function () {
      view.nextBtnNested();
    });
    this.addClickEvent("btnNestedLoop", function () {
      view.switchMultipleDivForNestedLoop();
    });
    this.addClickEvent("btnSimpleLoop", function () {
      view.switchMultipleDivForSimpleLoop();
    });
    this.addEventOnInputBox("simpleLoopInput", function () {
      view.getInput();
    });
    this.addEventOnInputBox("nestedLoopInput", function () {
      view.getNestedInput();
    });
  },
  ...
  displayLoop: function (loopId, firstStatementId) {
    ...
    for (i = 1; i < allChild.length; i += 2) {
      ...
      else this.changeClass(allChild[i].id, "showDiv");
    }
  },
  ...
  startBtn: function () {
    ...
    var inputValue = document.getElementById("simpleLoopInput").value;
    if (inputValue === "" || isNaN(model.inp) || model.inp === 0) {
      alert(
        "Please enter a valid, non-zero input value to start the simulation.",
      );
      return;
    }
  ...
  nextBtn: function () {
    ...
    if (model.inp === 0) {
      if (
        this.lastRedDiv.id !== "codeContentFor6" &&
        this.lastRedDiv.id !== "codeContentWhile6" &&
        this.lastRedDiv.id !== "codeContentDoWhile7"
      )
        this.highlightNextStep();
      else {
      ...
      if (
        this.lastRedDiv.id === "codeContentFor4" ||
        this.lastRedDiv.id === "codeContentWhile4" ||
        this.lastRedDiv.id === "codeContentDoWhile5"
      ) {
  ...
  startBtnNested: function () {
    ...
    this.clearDivs();
    var inputValue = document.getElementById("nestedLoopInput").value;
    if (inputValue === "" || isNaN(model.nestedInp) || model.nestedInp === 0) {
      alert(
        "Please enter a valid, non-zero input value to start the nested loop simulation.",
      );
      return;
    }
  ...
  nextBtnNested: function () {
    ...
    if (model.nestedInp === 0) {
      if (this.lastRedDiv.id !== "codeContentNested4") this.highlightNextStep();
      else this.processNestedLoopStep("codeContentNested21");
      ...
      if (this.lastRedDiv.id === "codeContentNested9" && this.j < this.copy)
        ...
        this.lastRedDiv.id === "codeContentNested6" &&
        this.j < this.copy
      )
        ...
        if (this.k < this.n) this.displayLocalVariableK();
        else this.processNestedLoopStep("codeContentNested15");
      } else if (this.lastRedDiv.id === "codeContentNested12") this.printStar();
      else if (this.lastRedDiv.id === "codeContentNested13") {
        this.printSpace();
        this.highlightNextStep();
      } else if (this.lastRedDiv.id === "codeContentNested14")
        this.processNestedLoopStep("codeContentNested10");
      else if (this.lastRedDiv.id === "codeContentNested15") this.printStar();
      else if (
        this.lastRedDiv.id === "codeContentNested19" &&
        this.m < this.copy
      )
        this.printSpacesInSecondHalf();
      else if (
        this.lastRedDiv.id === "codeContentNested16" &&
        this.m < this.copy
      )
        ...
        if (this.i <= model.nestedInp) this.displayLocalVariableI();
        else this.endTheNestedLoopCode();
